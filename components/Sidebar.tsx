...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, currentView, onNavigate }) => {
  ...
  const getNavItemClasses = (isActive: boolean) => {
    return `w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-200 text-left group border ${
      isActive
        ? 'bg-blue-500/10 text-cyan-400 border-blue-500/20 shadow-[0_0_10px_rgba(6,182,212,0.1)]'
        : 'border-transparent text-slate-400 hover:text-white hover:bg-slate-900'
    }`;
  };

  return (
    <div className="hidden md:flex flex-col w-64 h-screen bg-slate-950 border-r border-slate-800 fixed left-0 top-0 z-50 overflow-hidden">
      
      {/* Premium Glow Effect - Right Edge */}
      {/* 1. Subtle blue wash entering from the right */}
      <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-cyan-500/5 to-transparent pointer-events-none" />
      
      {/* 2. Vertical highlight line along the border */}
      <div className="absolute inset-y-0 right-0 w-[1px] bg-gradient-to-b from-transparent via-cyan-400/40 to-transparent shadow-[0_0_15px_1px_rgba(34,211,238,0.2)] pointer-events-none" />
      
      {/* 3. Central bright spot for "High Tech" feel */}
      <div className="absolute top-1/2 right-0 -translate-y-1/2 w-[2px] h-32 bg-cyan-500/20 blur-lg pointer-events-none" />
      ...
      <nav className="flex-1 px-4 py-6 space-y-2 relative z-10">
        ...
        <button 
          ...
          className={getNavItemClasses(currentView === 'editor')}
        >
          <ImageIcon size={18} className="group-hover:text-cyan-400 transition-colors" />
          <span className="font-medium">Virtual Staging</span>
        </button>

        <button 
          onClick={() => onNavigate('dashboard')}
          className={getNavItemClasses(currentView === 'dashboard' || currentView === 'project')}
        >
          <LayoutDashboard size={18} className="group-hover:text-cyan-400 transition-colors" />
          <span className="font-medium">Projects</span>
        </button>

        <button className={getNavItemClasses(false)}>
      ...
      <div className="px-4 mb-6 relative z-10">
        <div className="p-4 rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 relative overflow-hidden group">
            ...
            <button className="mt-3 w-full py-1.5 text-xs font-medium text-white bg-slate-700 hover:bg-slate-600 rounded transition-colors border border-slate-600">
                Top up credits
            </button>
        </div>
      </div>
