...
import { X, Building2, Gavel, FileText, AlertTriangle, MapPin, Info, ChevronDown, Check, Users, Save } from 'lucide-react';
...
const EditProjectModal: React.FC<EditProjectModalProps> = ({ project, onClose, onUpdate }) => {
  ...
  // Legal / Status
  const [constructionStatus, setConstructionStatus] = useState<ConstructionStatus>(project.constructionStatus);
  ...
  const [isAgeRestricted, setIsAgeRestricted] = useState(project.isAgeRestricted);
  
  // Auction
  const [isAuction, setIsAuction] = useState(project.isAuction);
  const [auctionDetails, setAuctionDetails] = useState<AuctionDetails>(project.auctionDetails || {
    date: '',
    type: '',
    premium: ''
  });
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl animate-fade-in flex flex-col">
        ...
        <div className="flex justify-between items-center px-8 py-6 border-b border-slate-800 sticky top-0 bg-slate-900/95 backdrop-blur z-20">
          <div>
            <h2 className="text-2xl font-bold text-white tracking-tight">Edit Property Details</h2>
            <p className="text-sm text-slate-400 mt-1">Update legal facts and AI constraints.</p>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
                <label className="text-sm font-medium text-slate-300">
                    Permanent features or rooms to highlight
                </label>
                <textarea 
                    rows={3}
                    value={features}
